    ...
    <section className="mx-auto flex items-center justify-center w-fit py-32 md:py-64">
      <div className="p-2 md:p-4 relative border border-neutral-200 w-fit">
        {cornerPositions.map((corner, index) => (
          <PlusIcon key={index} className={`absolute ${corner.position} w-4 h-4`} />
        ))}
        <div className="w-full relative overflow-hidden py-4 md:py-8 px-4 md:px-16 rounded-xl bg-linear-to-b from-green-400 to-green-700 flex items-center justify-center shadow-[inset_0px_2px_4px_rgba(255,255,255,0.6)]">
          ...
          <span  className="text-2xl md:text-4xl text-center sm:text-left xl:text-6xl font-medium font-inter text-neutral-200/80">
            "We ain't no <span className="text-neutral-200 underline font-semibold italic">Nokia</span>
            ,<br /> yet we connecting people."
          </span>
        </div>
      </div>
    </section>
