...
const CircleAvatar = ({
  ...
  <motion.div
    layout={person.id !== 6}
    style={{
      zIndex: person.id === 6 ? 100 : 1,
      boxShadow: person.id === 6 ? '0 0 10px 0 rgba(0, 0, 0, 0.1)' : 'none',
      opacity: opacity,
    }}
...
interface AudioCallBoxProps {
  className?: string;
}

export const AudioCallBox = ({ className = '' }: AudioCallBoxProps) => {
            ...
            {people.map((person) => {
              ...
              return (
                ...
                >
                  <CircleAvatar
                    person={person}
                    opacity={person.id === 6 ? 1 : isHovered ? 1 : 0.5}
                  />
                </motion.div>
              );
            })}
