...
const File = ({ isHovered }: FileProps) => (
  <motion.div
    animate={{
...
const Video = ({ isHovered }: VideoProps) => (
  <motion.div
    animate={{
      y: isHovered ? -10 : 0,
  ...
  >
    <div className="size-full bg-neutral-200 rounded-md overflow-hidden relative p-2 gap-2 flex flex-col items-center justify-center">
      <div className="size-full bg-white rounded-sm overflow-hidden relative p-1">
        <StripedPattern
          className="mask-[radial-gradient(300px_circle_at_center,white,transparent)] text-neutral-200"
        />
      </div>
...
const Image = ({ isHovered }: ImageProps) => (
  <motion.div
    animate={{
      rotate: isHovered ? -8 : -4,
      x: isHovered ? -60 : -10,
  ...
  >
    <div className="size-full bg-neutral-200 rounded-md overflow-hidden relative p-2 gap-2 flex flex-col items-center justify-center">
      <div
        className="size-full bg-white rounded-sm shadow-sm overflow-hidden relative p-1"
        style={{
...
export const MessageTypeBox = ({ className = '' }: MessageTypeBoxProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full z-0 h-full scale-75 ease-out group-hover:scale-110 transition-all duration-300 opacity-40 group-hover:opacity-60 translate-y-1/2 absolute bottom-0 right-0 bg-[radial-gradient(circle,#22c55e,#4ade80,#86efac,#bbf7d0,transparent,transparent)] blur-3xl"></div>
      <div
