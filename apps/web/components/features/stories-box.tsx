...
export const StoriesBox = ({ className = '' }: StoriesBoxProps) => {
          ...
          <div className="w-full h-6 border-b border-neutral-200 flex items-center justify-between px-2 gap-1">
            {Array.from({ length: 7 }).map((_, index) => {
              ...
              return (
                <div
                  key={storyIndex}
                  onClick={() => setCurrentIndex(storyIndex)}
                  className={`w-4 h-4 transition-all duration-300 bg-neutral-200 rounded-full border border-neutral-300 ${
                    currentIndex === storyIndex ? 'group-hover:bg-green-200' : ''
                  }`}
                ></div>
              );
            })}
            ...
            <div className="size-full bg-stone-200 rounded-md p-2 pb-0 border border-neutral-300">
              <div className="size-full border border-neutral-300 bg-neutral-100 rounded-md p-2 flex items-center justify-center">
                <Image
                  ...
                  width={96}
                  height={96}
                  className="size-24 blur-sm group-hover:blur-none transition-all duration-300 object-cover rounded-md"
                />
              </div>
            </div>
          </div>
