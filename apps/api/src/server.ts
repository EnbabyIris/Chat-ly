...
class ChatTurboServer {
  ...
  async start(): Promise<void> {
    try {
      ...
      const { scheduleStatusCleanup } = await import(
        "./jobs/status-cleanup.job"
      );
      ...
      this.httpServer.listen(config.server.port, config.server.host, () => {
        ...
        if (isDevelopment()) {
          logger.info("Development server endpoints available", {
            ...
            documentation: "http://localhost:5001/api/v1",
          });
        }
      });
      ...
      this.httpServer.on("error", (error: NodeJS.ErrnoException) => {
        if (error.code === "EADDRINUSE") {
          ...
          console.error(
            `❌ Permission denied to bind to port ${config.server.port}`,
          );
          console.error(
