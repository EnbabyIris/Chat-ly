...
export class AdvancedRateLimiter {
  ...
  async checkLimit(key: string): Promise<RateLimitInfo> {
    ...
    if (userLimit.blocked && userLimit.blockUntil && userLimit.blockUntil > now) {
      return {
    ...
    const remainingPoints = Math.max(0, this.config.maxRequests - userLimit.count)
   ...
   * Track a request
   */
  private trackRequest(key: string, req: Request, res: Response): void {
  ...
  removeFromWhitelist(ip: string): void {
    if (this.config.whitelist) {
      this.config.whitelist = this.config.whitelist.filter(item => item !== ip)
    }
  }
  ...
  removeFromBlacklist(ip: string): void {
    if (this.config.blacklist) {
      this.config.blacklist = this.config.blacklist.filter(item => item !== ip)
    }
  }
  ...
  getStatistics(): {
    ...
    stats.averageRequestsPerKey = stats.totalActiveKeys > 0 
...
export const createRateLimiters = () => {
  return {
    ...
    sensitive: new AdvancedRateLimiter({
      ...
      keyGenerator: (req) => `sensitive_${req.ip}_${req.user?.id || 'anonymous'}`,
    }),
  }
}
 ...
 */
export const rateLimitMiddleware = (type: 'auth' | 'api' | 'static' | 'sensitive' | 'custom', customConfig?: RateLimitConfig) => {
