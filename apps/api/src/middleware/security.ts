...
export const apiVersioning = (
  ...
  const version =
    req.headers["accept-version"] || req.headers["api-version"] || "v1";
...
export const sqlInjectionProtection = (
  ...
  const checkValue = (value: any, key?: string): boolean => {
    if (key && sensitiveFields.has(key)) return false;
...
export const securityLogger = (
  ...
  res.on("finish", () => {
    ...
    if (statusCode === 429) {
      ...
      console.info(
        `⚠️ Client error: ${statusCode} - ${req.method} ${req.path} (${duration}ms)`,
      );
    }
